angular
  .module('app.core')
  .directive('registerFormAlt',registerFormAlt);

registerFormAlt.$inject = ['$window','userApi','Dialog', 'accountApi','rateApi','$q', '$state'];

function registerFormAlt($window,userApi, Dialog, accountApi,rateApi,$q, $state){
  
  return {
    restrict: 'E',
    templateUrl: 'app/directives/auth/registerAlt/register-alt.form.html',
    scope: true,
    link:function(scope,element,attr){
      ...
      scope.register = function(){
        if(scope.registerFormAlt.$valid){
          ...
          Dialog.confirmDialog(title,content,buttons, function(){
            ...
            validateTaxId(scope.account.taxId).then(function(){
              ...
              if(user){
                ...
                Dialog.successDialog({main:'¡Tu cuenta ha sido creada exitosamente!'}, {main: "Ahora puedes visitar tu dashboard"}, {main:{cancel:"Iniciar sesión"}}, () => {
                  $state.go("login")
                })
              }
